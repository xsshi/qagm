...
def pair_split(node_features, ns):
    # node_features (num, features), 按照ns分割features，总和为num
    # 按行分割，存入列表partition中
    # 在从partition中选取相邻两个分区分配到tgt和src中
    batch_feature_src = []
    batch_feature_tgt = []
    partitions = []
    idx = 0
...
def get_graph_feature(batch_graphs_src, ns_src, batch_graphs_tgt, ns_tgt):
    ...
    # 对角块矩阵，从邻接矩阵列表创建，将所有给定矩阵置于更大矩阵的对角线上，
