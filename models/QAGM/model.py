...
from models.QAGM.sconv_archs import SiameseSConvOnNodes, SiameseNodeFeaturesToEdgeFeatures
...
class Backbone(CNN):
    def __init__(self):
        ...
        # 节点特征和边特征提取
        self.message_pass_node_features = SiameseSConvOnNodes(input_node_dim=cfg.QAGM.FEATURE_CHANNEL * 2)
        ...
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / cfg.QAGM.SOFTMAXTEMP))
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.momentum = cfg.QAGM.MOMENTUM  # momentum parameter for the momentum network
        self.backbone_params = list(self.onlineNet.backbone_params) # used in train_eval.py
        self.warmup_step = cfg.QAGM.WARMUP_STEP  # warmup steps for the distillation
    ...
    def forward(self, data_dict, training=False, iter_num=0, epoch=0):
        ...
        if epoch * self.epoch_iters + iter_num >= self.warmup_step:
            alpha = cfg.QAGM.ALPHA
        else:
            alpha = cfg.QAGM.ALPHA * min(1, (epoch * self.epoch_iters + iter_num) / self.warmup_step)
        ...
        if training == True:
            ...
            quadraloss = Distill_QuadraticAssignmentContrast()
            ql = quadraloss(node_feature_list, node_feature_m_list, alpha, self.onlineNet.logit_scale, self.momentumNet.logit_scale)
            loss = loss + ql  + crossloss(node_feature_list, node_feature_m_list, self.onlineNet.logit_scale, self.momentumNet.logit_scale)
