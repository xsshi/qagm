...
def build_graphs(P_np: np.ndarray, n: int, n_pad: int=None, edge_pad: int=None, stg: str='fc', sym: bool=True,
    ...
    assert stg in ('fc', 'tri', 'gabriel', 'near'), 'No strategy named {} found.'.format(stg)

    if stg == 'tri':
        A = delaunay_triangulate(P_np[0:n, :])
    elif stg == 'gabriel':  # 添加这一行以选择Gabriel图
...
def no_points_in_circle(P, i, j):
    """检查是否有其他点在点i和点j定义的圆内"""
    midpoint = (P[i] + P[j]) / 2
    radius = np.linalg.norm(P[i] - P[j]) / 2
    for k in range(len(P)):
        if k != i and k != j and np.linalg.norm(P[k] - midpoint) < radius:
            return False
    return True
...
def gabriel_graph(P: np.ndarray) -> np.ndarray:
    """
    Generate a Gabriel graph from a set of points.

    :param P: (n x 2) NumPy array of point coordinates.
    :return: Adjacency matrix A where A[i, j] is 1 if an edge exists between points i and j.
    """
    n = P.shape[0]
    A = np.zeros((n, n), dtype=int)
    for i in range(n):
        for j in range(i + 1, n):
            if no_points_in_circle(P, i, j):
                A[i, j] = A[j, i] = 1
    return A
