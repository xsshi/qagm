...
class Distill_QuadraticAssignmentContrast(torch.nn.Module):
    def __init__(self):
        super(Distill_QuadraticAssignmentContrast, self).__init__()

    def forward(self, feature:Tensor, feature_m: Tensor, alpha:float, dynamic_temperature: Tensor, dynamic_temperature_m: Tensor) -> Tensor:
        graph1_feat = F.normalize(feature[0], dim = -1)
        graph2_feat = F.normalize(feature[1], dim = -1)
        batch_size = graph1_feat.shape[0]

        # calculate the edge adjacent matrix

        # adjacent_matrix_1 = dynamic_temperature.exp() * graph1_feat @ graph1_feat.T
        # adjacent_matrix_2 = dynamic_temperature.exp() * graph2_feat @ graph2_feat.T

        adjacent_matrix_1 = graph1_feat @ graph1_feat.T
        adjacent_matrix_2 = graph2_feat @ graph2_feat.T
        #
        # adjacent_matrix_1.fill_diagonal_(0)
        # adjacent_matrix_2.fill_diagonal_(0)

        # calculate the eigenvalue of adjacent matrix



        eigenvalue_1, _ = torch.symeig(adjacent_matrix_1, eigenvectors = True)
        eigenvalue_2, _ = torch.symeig(adjacent_matrix_2, eigenvectors = True)

        # the size of two graph and the number of eigenvalues
        len_1 = eigenvalue_1.size(0)
        len_2 = eigenvalue_2.size(0)

        max_len = max(len_1, len_2)

        # add 0 to the shorter eigenvalue vector
        if len_1 < max_len:
            padding = (0, max_len - len_1)
            eigenvalue_1 = F.pad(eigenvalue_1, padding, mode='constant', value=0)

        if len_2 < max_len:
            padding = (0, max_len - len_2)
            eigenvalue_2 = F.pad(eigenvalue_2, padding, mode='constant', value=0)

        # sort the eigenvalue vector increasing and decreasing
        eigenvalue_1_sorted, _ = torch.sort(eigenvalue_1)
        eigenvalue_2_sorted, _ = torch.sort(eigenvalue_2)

        # eigenvalue_1_sorted_reverse = eigenvalue_1_sorted.flip(dims=[0])
        ...
        with torch.no_grad():
            ...
            adjacent_matrix_1_m = graph1_feat_m @ graph1_feat_m.T
            adjacent_matrix_2_m = graph2_feat_m @ graph2_feat_m.T
            # online similarity

            eigenvalue_1_m, _ = torch.symeig(adjacent_matrix_1_m, eigenvectors = True)
            eigenvalue_2_m, _ = torch.symeig(adjacent_matrix_2_m, eigenvectors = True)

            len_1_m = eigenvalue_1_m.size(0)
            len_2_m = eigenvalue_2_m.size(0)

            max_len_m = max(len_1_m, len_2_m)

            if len_1_m < max_len_m:
                padding = (0, max_len_m - len_1_m)
                eigenvalue_1 = F.pad(eigenvalue_1_m, padding, mode='constant', value=0)

            if len_2_m < max_len_m:
                padding = (0, max_len_m - len_2_m)
                eigenvalue_2 = F.pad(eigenvalue_2_m, padding, mode='constant', value=0)

            # sort the eigenvalue vector increasing and decreasing of momentum
            eigenvalue_1_sorted_m, _ = torch.sort(eigenvalue_1_m)
            eigenvalue_2_sorted_m, _ = torch.sort(eigenvalue_2_m)


        quadra_loss =   (alpha*torch.dot(eigenvalue_1_sorted_m, eigenvalue_2_sorted_m) + (1-alpha)*torch.dot(eigenvalue_1_sorted, eigenvalue_2_sorted)) / (max_len * max_len)
