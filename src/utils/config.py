...
def _merge_a_into_b(a, b):
    ...
    for k, v in a.items():
        ...
        # If b expects a tuple and a provides a list, convert list to tuple
        if type(b[k]) is tuple and type(v) is list:
            v = tuple(v)
        elif type(b[k]) is not type(v):
            if type(b[k]) is float and type(v) is int:
                v = float(v)  # Convert int to float if expected
            else:
                if k not in ['CLASS']:  # Allow 'CLASS' to bypass type check if needed
                    raise ValueError('Type mismatch ({} vs. {}) for config key: {}'.format(type(b[k]), type(v), k))

        # Recursively merge dicts if they are edict instances
        if type(v) is edict:
            try:
